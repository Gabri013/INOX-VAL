...
function buildSheetModule(sheet: SheetModel) {
  const entries = JSON.stringify(sheet.cells, null, 2);
  return [
    '// Auto-generated. Do not edit by hand.',
    "import type { SheetData } from '../types.ts';",
    '',
    'export const sheetName = ' + JSON.stringify(sheet.name) + ';',
    'export const cells: SheetData = ' + entries + ';',
    '',
  ].join('\n');
}

function buildEngineModule(sheetFiles: { name: string; file: string }[]) {
  const imports = sheetFiles
    .map((s, i) => "import * as sheet" + String(i) + " from './sheets/" + s.file + "';")
    .join('\n');
  const mapEntries = sheetFiles
    .map((s, i) => '  ' + JSON.stringify(s.name) + ': sheet' + String(i) + '.cells,')
    .join('\n');
...
function buildRuntimeModule() {
  return `// Auto-generated runtime for planilha engine.
...
function buildTypesModule() {
  return `export type CellEntry = { value: string | number | boolean | null; formula?: string };
...
function main() {
  const modelPath = process.argv[2] || 'src/domains/precificacao/embedded/planilha.model.json';
  if (!fs.existsSync(modelPath)) {
    throw new Error(`Model nao encontrado: ${modelPath}`);
  }

  const model = JSON.parse(fs.readFileSync(modelPath, 'utf8')) as WorkbookModel;
  const engineDir = path.resolve('src/domains/precificacao/engine');
  const sheetsDir = path.join(engineDir, 'sheets');
  ensureDir(sheetsDir);

  const sheetFiles: { name: string; file: string }[] = [];
  model.sheets.forEach((sheet, index) => {
    const fileName = "sheet_" + String(index + 1) + ".ts";
    writeFile(path.join(sheetsDir, fileName), buildSheetModule(sheet));
    sheetFiles.push({ name: sheet.name, file: fileName });
  });

  writeFile(path.join(engineDir, 'types.ts'), buildTypesModule());
  writeFile(path.join(engineDir, 'runtime.ts'), buildRuntimeModule());
  writeFile(path.join(engineDir, 'engine.ts'), buildEngineModule(sheetFiles));

  console.log('Engine gerada.');
}
