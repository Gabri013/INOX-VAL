...
export default function Orcamentos() {
  ...
  const columns = [
    ...
    },
    {
      ...
      label: "Data",
      sortable: true,
      render: (orc: Orcamento) => format(orc.data, "dd/MM/yyyy", { locale: ptBR })
    },
    {
      key: "clienteNome",
      label: "Cliente",
    ...
    },
    {
      ...
      render: (orc: Orcamento) => (
        <Badge variant="outline">{orc.itens.length}</Badge>
      )
    },
    {
      key: "validade",
      ...
      sortable: true,
      render: (orc: Orcamento) => {
        const diasRestantes = Math.ceil((orc.validade.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
        const vencido = diasRestantes < 0;
        
        return (
          <div className="text-sm">
            <div>{format(orc.validade, "dd/MM/yyyy", { locale: ptBR })}</div>
