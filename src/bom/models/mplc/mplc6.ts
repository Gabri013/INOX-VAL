...
export function gerarBOM_MPLC6(config: MesaConfig): BOMResult {
  ...
  // 2. ESTRUTURA CONTRAVENTADA (6 pés)
  ...
  bom.push({
    ...
    pesoTotal: calcularPesoTubo(comprimentoTravessaTransv) * 6,
    custo: calcularCustoTubo(comprimentoTravessaTransv),
    custoTotal: calcularCustoTubo(comprimentoTravessaTransv) * 6,
    unidade: 'pç',
  });
  ...
  bom.push({
    ...
    pesoTotal: calcularPesoTubo(comprimentoDiagonal) * 6,
    custo: calcularCustoTubo(comprimentoDiagonal),
    custoTotal: calcularCustoTubo(comprimentoDiagonal) * 6,
    unidade: 'pç',
  });
